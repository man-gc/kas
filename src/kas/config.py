...
from .includehandler import IncludeException, IncludeHandler
...
class Config:
    ...
    def __init__(self, ctx, filename, target=None, task=None):
        ...
        self.filenames = [os.path.abspath(configfile) for configfile in filename.split(':')]
        top_repo_path = Repo.get_root_path(os.path.dirname(self.filenames[0]))

        repo_paths = [Repo.get_root_path(os.path.dirname(configfile), fallback=False) for configfile in self.filenames]

        if len(set(repo_paths)) > 1:
            raise IncludeException(
                'All concatenated config files must '
        ...
        self.handler = IncludeHandler(self.filenames, top_repo_path, not update)
    ...
    def find_missing_repos(self, repo_paths=None):
        """
        ...
        (self._config, missing_repo_names) = self.handler.get_config(repos=repo_paths)
    ...
    def get_repo(self, name):
        ...
        overrides = self._config.get('overrides', {}).get('repos', {}).get(name, {})
        config = self.get_repos_config()[name] or {}
        top_repo_path = self.handler.get_top_repo_path()
        return Repo.factory(name, config, repo_defaults, top_repo_path, overrides)
    ...
    def get_bitbake_targets(self):
        ...
        environ_targets = [i for i in os.environ.get('KAS_TARGET', '').split() if i]
        if environ_targets:
    ...
    def get_bitbake_task(self):
        ...
        return os.environ.get('KAS_TASK', self._config.get('task', 'build'))
    ...
    def get_machine(self):
        ...
        Returns the machine
        """
        return os.environ.get('KAS_MACHINE', self._config.get('machine', 'qemux86-64'))

    def get_distro(self):
        """
        Returns the distro
        """
        return os.environ.get('KAS_DISTRO', self._config.get('distro', 'poky'))
